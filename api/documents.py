...
class Documents(Handler):
    ...
    def get(self, city = "Aachen"):
        ...
        print city
        documents = self.app.mongodb.submissions.find()
        print documents.count()
        documents2 = self.app.mongodb.submissions.find(
            {
                'ERROR': {"$exists": False},
        ...
        if (offset + limit) <= count:
            metadata['next'] = self.url_for("documents", city = city.lower(), offset = limit+offset, _full = True, _append = True, **args)
        if offset != 0:
            metadata['prev'] = self.url_for("documents", city = city.lower(), offset = offset-limit, _full = True, _append = True, **args)
        result = {
            '_meta' : metadata,
